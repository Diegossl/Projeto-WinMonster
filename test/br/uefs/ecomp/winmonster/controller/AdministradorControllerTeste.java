...
public class AdministradorControllerTeste {
	...
	public void testCompactarComSucesso(){
		...
		Fila fila = null;
		try {
			fila = controllerAdm.filaDeFrequencias(arquivoOriginal);
		} catch (IOException e) {
			fail();
		} catch (ArquivoVazioException e) {
			fail();
		...
		try {
			controllerAdm.compactar(raiz, textoCodificado,arquivoOriginal.getPath().replace(arquivoOriginal.getName(), ""), arquivoOriginal.getName(), controllerAdm.funcaoHash(controllerAdm.getHuff().getTextoOriginal()));
		} catch (IOException e) {
			fail();
		}
		...
		if(arquivoCompactado.length() < arquivoOriginal.length())
			chave2 = 1;
